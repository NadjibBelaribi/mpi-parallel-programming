...
mnt *darboux(const mnt *restrict m)
{
  ...
  while (modif)
  {
    ...
    {
      // send last to next
      MPI_Send(&Wprec[(nrows -2)*ncols],ncols, MPI_FLOAT,rank+1, MPI_ANY_TAG, MPI_COMM_WORLD);

      // recv first of next
      MPI_Recv(&Wprec[nrows-1], ncols, MPI_FLOAT, rank + 1, MPI_ANY_TAG, MPI_COMM_WORLD, NULL);
    }
    else if (rank == size - 1)
    {
      // send first to precedent
      MPI_Send(&Wprec[ncols], ncols, MPI_FLOAT,rank-1,MPI_ANY_TAG, MPI_COMM_WORLD);

      // recv last of precedent
      MPI_Recv(&Wprec[0], ncols, MPI_FLOAT, rank - 1, MPI_ANY_TAG, MPI_COMM_WORLD, NULL);
    }
    else
    {
    ...
    //send last to next
      ...
      MPI_Recv(&Wprec[(nrows -1)*ncols], ncols, MPI_FLOAT, rank + 1, MPI_ANY_TAG, MPI_COMM_WORLD, NULL);
    }
